...
@customElement('fall-weather-observations')
export default class FallWeatherObservations extends LitElement {
  static styles = css`
    table {
      background: white;
      color: black;
  ...
  parseObservations(observations: Observation[]): {
    ...
    return {
      ...
      cutoff: new Date(`November 1, ${new Date().getFullYear()}`),
      averageTemp:
